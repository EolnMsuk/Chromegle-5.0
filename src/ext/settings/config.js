const config = {
    ...
    "countrySkipInfo": new CountrySkipField({
    // --- END OF CHANGE ---
        ...
        // *** THIS CHECK FUNCTION IS UPDATED ***
        "check": (response) => {
            // Early exit for null response (e.g., user cancelled prompt)
            if (response == null) {
                return { "confirm": "false", "value": null };
            }

            // If the response is empty, allow it (for clearing the list)
            if (response.trim() === "") {
            ...
            // Validate characters allowed: letters, commas, spaces
             if (!response.match(/^[a-zA-Z,\s]+$/)) {
                 ...
                 return { "confirm": "false", "value": null }; // Don't save invalid chars
             }
            ...
            for (let code of codes) {
                let trimmedCode = code.trim().toUpperCase(); // Trim spaces and uppercase

                if (trimmedCode.length === 0) {
                    continue; // Skip empty entries caused by extra commas ,,
                }
                ...
                if (trimmedCode.length < 2 || trimmedCode.length > 3) {
                    ...
                    validCodes.add(trimmedCode); // Add valid, trimmed, uppercase code
                }
            }

            if (invalidFound) {
                return { "confirm": "false", "value": null }; // Don't save if any code was invalid
            }

            // Join the validated, trimmed, uppercase codes with commas (no spaces)
            let finalValue = [...validCodes].join(",");

            return {
                "confirm": "true",
                "value": finalValue // Save the cleaned list
            };
        }
        // *** END OF UPDATED CHECK FUNCTION ***
    }),
