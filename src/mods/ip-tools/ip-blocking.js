...
class IPBlockAPI {
    ...
    async skipBlockedAddress(address) {
        ...
        if (shouldSkip) {
            ...
            // *** FIX: Get the element and add the cleanup class ***
            let errorMsgElement = sendErrorLogboxMessage(`Skipped the blocked IP address ${address}`); // Create the message
            if (errorMsgElement) { // Check if the element was created successfully
                 errorMsgElement.classList.add("chromegle-ip-logitem"); // Add the class needed for cleanup
                 errorMsgElement.appendChild(ButtonFactory.ipUnblockButton(address)); // Append the button
                 Logger.DEBUG("Added cleanup class to IP block skip message.");
            } else {
                 Logger.ERROR("Failed to create error message element for IP block skip.");
            }
            // *** End FIX ***

            // Pass the current chat's UUID to the skip function.
    ...
    async unblockAddress(address, logInChat = true) {
        ...
        if (!await this.#blockList.isBlocked(address)) {
            // You might want to remove this alert or change it,
            // as it will now show up immediately if the IP isn't blocked.
        ...
        // Log in chat
        if (logInChat) {
            // Consider changing this message if you remove the alert above
            sendErrorLogboxMessage(`Unblocked the IP address ${address} in video chat.`);
        }

        // Attempt to replace the button if it exists in the chat
    ...
    async blockAddress(address) {
        ...
        // No confirmation needed, proceed directly to blocking.

        // Check if blocked (still useful to prevent redundant actions/logging)
        if (await this.#blockList.isBlocked(address)) {
            // You might want to remove this alert or keep it for feedback
            alert(`The IP address ${address} is already blocked in video chat!`);
            // Even if already blocked, ensure the button shows "Unblock"
            const blockButton = document.getElementById("ipBlockButton");
            if (blockButton && typeof ButtonFactory !== 'undefined' && typeof ButtonFactory.ipUnblockButton === 'function') {
                 blockButton.replaceWith(ButtonFactory.ipUnblockButton(address));
            }
            return true; // Indicate it's blocked (or was already)
        }
        ...
        // Skip if chatting and send message
        try {
            // Check ChatRegistry safely
            if (typeof ChatRegistry !== 'undefined' && ChatRegistry.isChatting()) {
                 const currentUUID = ChatRegistry.getUUID(); // Get UUID before potential skip
                 // Check performDebouncedSkip safely
                 if (typeof performDebouncedSkip === 'function') {
                    performDebouncedSkip(currentUUID);
                 } else {
                      Logger.ERROR("performDebouncedSkip function not found!");
                 }
                // Use await for the message when manually blocking
                await sendErrorLogboxMessage(`Blocked the IP address ${address} and skipped the current chat.`);
            } else {
                // Use await for the message when manually blocking
                await sendErrorLogboxMessage(`Blocked the IP address ${address} in video chat.`);
            }
        } catch (e) { Logger.ERROR("Error sending block message or skipping:", e); }


        // Switch with unblock button safely
         try {
            // Check ButtonFactory safely
            if (typeof ButtonFactory !== 'undefined' && typeof ButtonFactory.ipUnblockButton === 'function') {
                // Find the button again in case it was re-added if already blocked
                document.getElementById("ipBlockButton")?.replaceWith(
                    ButtonFactory.ipUnblockButton(address)
                );
            } else {
                 Logger.WARNING("ButtonFactory not ready for unblock button replacement.");
            }
        } catch (e) { Logger.ERROR("Error replacing block button:", e); }


        return true; // Indicate block was successful
    }
